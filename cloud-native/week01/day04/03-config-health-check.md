...
환경 변수는 앱을 실행할 때 바깥에서 잠깐 전달하는 설정값이다. 코드 파일을 고치지 않고도 앱 이름, 포트, 데이터 파일 위치를 바꿀 수 있다.

Linux/macOS/WSL에서는 아래처럼 명령 앞에 붙여 실행할 수 있다.

```bash
APP_NAME=practice-shop python3 server.py
```

이 말은 “이번 실행에서만 `APP_NAME`을 `practice-shop`으로 보고 서버를 실행하라”는 뜻이다.

Windows PowerShell은 문법이 다르다.

```powershell
$env:APP_NAME="practice-shop"; py server.py
```

운영체제마다 명령 모양은 다르지만 핵심은 같다. 코드 파일을 고치지 않고 실행 시점의 설정을 바꾼다.
