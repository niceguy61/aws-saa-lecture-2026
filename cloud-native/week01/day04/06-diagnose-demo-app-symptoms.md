...
처음에는 이 순서만 지켜도 충분하다. 브라우저 화면만 보고 “앱이 죽었다”고 말하지 않고, 서버가 켜져 있는지와 로그가 찍히는지부터 확인한다.
...
## `500`과 `unhealthy`가 같이 보일 때

데이터 파일 경로를 틀리게 실행하면 `/api/products`는 `500`, `/health`는 `unhealthy`가 될 수 있다.

| 경로 | 보이는 증상 | 해석 |
|---|---|---|
| `/api/products` | `500` | 상품 데이터를 만들다가 실패 |
| `/health` | `unhealthy` | 앱이 필요한 데이터 파일을 못 찾아 운영 상태가 나쁨 |

둘은 서로 다른 문제가 아니라 같은 원인에서 나온 두 증상일 수 있다. 그래서 진단할 때는 증상 하나만 보지 말고 로그, API 응답, health 응답을 함께 본다.
