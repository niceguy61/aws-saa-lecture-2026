...
이 시간의 목표는 학생들이 AI 도구를 수업에서 안전하고 생산적으로 쓰는 기준을 세우는 것이다. ChatGPT 같은 대화형 도구와 CLI 기반 coding agent의 차이를 이해하고, AI가 제안한 내용을 검증하는 습관을 만든다.

이 세션의 끝에서 학생들은 다음을 말할 수 있어야 한다.

- AI 도구가 개발 학습에서 도움이 되는 지점과 위험한 지점을 구분한다.
- 대화형 AI와 coding agent의 차이를 설명한다.
...
| 시간 | 단계 | 진행 |
|---|---|---|
| 16:00-16:07 | 사용 현황 조사 | 학생들이 써본 AI 도구와 용도를 손들기/스티커로 공유한다. |
| 16:07-16:18 | AI 도구 변천사 | 자동완성, 챗봇, IDE assistant, coding agent로 이어진 흐름을 설명한다. |
| 16:18-16:30 | 도움이 되는 일과 위험한 일 | 에러 해석, 설명, 예제 생성, 명령 실행 위험을 분리한다. |
| 16:30-16:40 | CLI coding agent 소개 | 작업 디렉터리, 파일 변경, 명령 실행, 검증 흐름을 설명한다. |
| 16:40-16:47 | 안전 규칙 만들기 | 팀별로 AI 사용 규칙 3개를 작성한다. |
| 16:47-16:50 | 다음 시연 예고 | 8교시에 작은 예시를 실제로 보여준다고 안내한다. |
...
flowchart LR
  ...
  B --> C["제안<br/>명령 코드 문서"]
  C --> D{"검증 가능한가?"}
  D -- "아니오" --> E["질문을 작게 나누기<br/>공식 문서 확인<br/>강사에게 질문"]
  D -- "예" --> F["작은 범위에서 실행"]
  F --> G["변경 파일과 출력 확인"]
  G --> H["내 말로 기록"]
```
...
### 장면: AI와 함께하지만 검증하는 학생

- 이미지 파일: [assets/stickman-ai-code-review.png](assets/stickman-ai-code-review.png)
- 장면: 학생이 AI 말풍선에서 나온 명령을 바로 실행하지 않고 체크리스트로 확인한다.
- 인물: 노트북 앞 학생, 옆에서 리뷰하는 팀원, 안전 규칙을 가리키는 강사
- 기술 오브젝트: AI 말풍선, terminal command, changed files list, test result
- 감정: AI를 적극적으로 쓰되 주도권은 학생에게 있는 분위기
- 반드시 보여줄 것: 변경 파일과 테스트 결과 확인
- 피할 것: AI가 마법처럼 모든 것을 해결하는 느낌, 실제 secret 문자열
- Prompt:

```text
Simple black-and-white stickman illustration on a clean white background. A student reviews an AI assistant speech bubble that suggests a terminal command, while a teammate checks a changed-files list and a test result checklist. The instructor points to safety rules on a board. Minimal line art, classroom-friendly, no secrets, no brand logos, no dense UI.
```
...
“다음 시간에는 coding agent 예시를 짧게 시연합니다. 작은 파일을 만들고, 명령을 실행하고, 바뀐 파일을 확인하는 흐름을 보겠습니다.”
