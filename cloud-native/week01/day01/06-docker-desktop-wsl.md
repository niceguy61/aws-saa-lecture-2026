...
## 오늘 Docker를 어디까지 이해하면 되나

오늘의 목표는 Docker 전문가가 되는 것이 아니다. 아래 정도만 구분하면 된다.

| 말 | 오늘의 이해 |
|---|---|
| Docker Desktop | 내 노트북에서 Docker Engine을 켜고 관리하는 앱 |
| Docker CLI | 터미널에서 `docker ...` 명령을 보내는 도구 |
| Docker Engine | 실제로 이미지와 컨테이너를 다루는 실행 담당 |
| image | 컨테이너를 만들기 위한 실행 재료 |
| container | 이미지에서 만들어져 실제로 실행되는 것 |

이미지와 컨테이너의 자세한 차이, Dockerfile 작성법, 포트 매핑은 2주차에 배운다. 오늘은 “CLI가 Engine과 연결되는지”, “작은 컨테이너를 한 번 실행할 수 있는지”를 확인하는 날이다.
