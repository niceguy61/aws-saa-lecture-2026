...
Coding agent가 완료했다고 말해도, 변경 파일과 실행 결과를 직접 확인해야 작업이 끝난 것이다.
...
2일차에는 Python으로 만든 아주 작은 웹앱을 실행한다. 웹 개발자가 되기 위한 실습이 아니라, Cloud Native에서 계속 등장하는 실행 단위를 직접 보기 위한 실습이다.

확인할 것:

| 항목 | 확인 방법 | 막혔을 때 |
|---|---|---|
| Python | `python --version` 또는 `python3 --version` | 2일차 첫 시간 설치 안내를 따라간다 |
...
Python이 아직 없어도 괜찮다. 내일 첫 시간에 설치와 확인을 먼저 진행한다. 다만 본인 컴퓨터가 Windows인지 macOS인지, 관리자 권한이 있는지, 회사/학교 보안 프로그램 때문에 설치가 막히는지는 미리 확인해두면 좋다.
