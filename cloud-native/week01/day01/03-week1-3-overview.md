...
이 시간의 목표는 1-3주차를 하나의 이야기로 이해하는 것이다. 학생들이 아직 Docker와 Kubernetes를 자세히 몰라도, 왜 프로세스와 포트부터 시작하는지, 왜 컨테이너를 배우는지, 왜 Kubernetes까지 가는지 연결할 수 있어야 한다.

이 세션의 끝에서 학생들은 다음을 말할 수 있어야 한다.

- 1주차 컴퓨팅 기초가 Docker와 Kubernetes 학습의 바닥 지식임을 설명한다.
...
| 시간 | 단계 | 진행 |
|---|---|---|
| 11:00-11:07 | 출발점 확인 | “앱이 실행된다”는 말 안에 무엇이 들어있는지 묻는다. |
| 11:07-11:18 | 1주차 이야기 | 프로세스, 포트, 파일, 환경 변수, 로그가 왜 필요한지 설명한다. |
| 11:18-11:30 | Docker가 생긴 이유 | 실행 환경 차이와 배포 불안을 역사 흐름으로 설명한다. |
| 11:30-11:40 | Kubernetes가 생긴 이유 | 컨테이너가 많아지며 생기는 배치, 재시작, 확장, 연결 문제를 설명한다. |
| 11:40-11:47 | 1-3주차 지도 그리기 | 학생들이 각 주차의 키워드를 서비스 흐름 위에 붙인다. |
...
강사는 답을 모아 “프로세스, 포트, 요청, 파일, 환경 변수, 로그”로 연결한다.
...
- 이미지 파일: [assets/stickman-week1-3-map.png](assets/stickman-week1-3-map.png)
- 장면: 왼쪽에는 학생의 노트북 앱, 가운데에는 컨테이너 박스, 오른쪽에는 여러 컨테이너를 정리하는 플랫폼 보드가 있다.
- 인물: 노트북 앞 학생, 컨테이너를 포장하는 팀원, 운영 보드를 보는 강사
- 기술 오브젝트: 프로세스 아이콘, 포트 표지판, 컨테이너 상자, Pod 카드
- 감정: 복잡한 기술이 한 단계씩 이어진다는 느낌
- 반드시 보여줄 것: 1주차, 2주차, 3주차가 길처럼 이어지는 장면
- 피할 것: 실제 Kubernetes 로고, 작은 글씨가 많은 아키텍처
- Prompt:

```text
Simple black-and-white stickman illustration on a clean white background. Show a learning path from a laptop app, to a container box, to a simple platform board with several container cards. Students move sticky notes labeled process, port, image, container, pod, and service along the path. Use small blue arrows. Minimal line art, classroom-friendly, no brand logos, no dense UI.
```
