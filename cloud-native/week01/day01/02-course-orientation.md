...
이 시간의 목표는 학생들이 “이 수업에서 무엇을 어떻게 배우는지”를 예측할 수 있게 만드는 것이다. Cloud Native는 Docker 명령어 몇 개를 외우는 과정이 아니라, 코드가 실제 서비스가 되는 과정을 반복해서 관찰하고 설명하는 과정이다.

이 세션의 끝에서 학생들은 다음을 말할 수 있어야 한다.

- 이 과정이 Docker, Kubernetes, AWS, Observability, AI를 한 흐름으로 다루는 이유를 설명한다.
...
| 시간 | 단계 | 진행 |
|---|---|---|
| 10:00-10:06 | 1교시 연결 | 학생들이 적은 설치/도구 걱정 중 공통 항목을 2-3개 소개한다. |
| 10:06-10:15 | 과정의 큰 약속 | 이 과정은 “앱을 서비스로 만드는 길”을 반복해서 보는 수업이라고 설명한다. |
| 10:15-10:25 | 왜 이런 과정이 생겼나 | 업계가 서버 구매, 수동 배포, 클라우드, 컨테이너, 플랫폼 운영으로 이동한 흐름을 소개한다. |
| 10:25-10:35 | 수업 운영 방식 | 보기, 설명하기, 해보기, 진단하기, 회고하기 루프를 설명한다. |
...
학생들에게 손들기로 먼저 묻는다. 어느 쪽이 맞는지 바로 판정하지 않는다. 이 과정에서는 두 능력이 연결된다는 점을 뒤에서 설명한다.
...
질문은 아래 템플릿으로 받는다. 완벽하게 말할 필요는 없지만, 가능하면 증상과 시도한 일을 같이 말한다.
...
학생들에게 아래 문장을 보여주고 2분 동안 더 좋은 질문으로 바꾸게 한다.
...
- 이미지 파일: [assets/stickman-good-question.png](assets/stickman-good-question.png)
- 장면: 한 학생이 “안 돼요”라고 적힌 말풍선을 들고 있고, 팀원이 옆에서 증상, 시도한 일, 질문으로 나누어 적는다.
- 인물: 질문하는 학생, 기록하는 학생, 칠판을 가리키는 강사
- 기술 오브젝트: 터미널 창, 체크리스트, 작은 서비스 흐름도
- 감정: 평가받는 분위기가 아니라 같이 문제를 좁혀가는 분위기
- 반드시 보여줄 것: 질문이 구조화되는 장면
- 피할 것: 실제 에러 로그의 작은 글씨, 특정 회사 로고
- Prompt:

```text
Simple black-and-white stickman illustration on a clean white background. A student holds a speech bubble that says "It does not work" while teammates transform it into a checklist with symptom, tried steps, and question. The instructor points to a simple service flow on a whiteboard. Minimal line art, classroom-friendly, no brand logos, no dense text.
```
