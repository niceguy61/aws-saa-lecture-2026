...
npm install이 완료된 후 `npm run dev` 명령어 실행 시 컨테이너가 즉시 종료되거나 실행되지 않는 경우입니다. 이는 컨테저 내부 파일 시스템 접근 권한 또는 경로 설정 문제로 인해 발생합니다.

### 💬 개념 설명

**워킹 디렉토리**: 프로젝트 파일을 저장하는 폴더입니다. 예를 들어, `/app` 디렉토리가 워킹 디렉토리라면 `npm run dev`는 이 폴더에서 실행됩니다. 이는 프로그래밍에서 "작업 공간"처럼 생각해보세요.

**바인드 마운트**: 컴퓨터와 컨테이너가 파일을 공유하는 방식입니다. 개발 중에는 호스트의 파일 변경사항이 컨테이너에 실시간으로 반영됩니다. 이는 "파일 공유 기능"이라고 생각해보세요.

**권한 설정**: 파일/디렉토리에 접근할 수 있는 권한을 조절하는 설정입니다. 컨테이너가 특정 파일에 쓰기 권한이 없으면 실행이 중단됩니다.
...
3. `docker-compose up --build` 명령어로 이미지 재빌드 (변경 사항 반영)  
   → "변경 사항을 반영하고 이미지 다시 생성"  
4. `chmod -R 777 /app` 명령어로 컨테이너 디렉토리 권한 재설정 (권한 문제 해결)  
   → "임시로 권한을 조정하고, 실제 환경에서는 보안 설정을 고려해야 함"  
...
**경로 매핑**: 호스트 컴퓨터의 디렉토리를 컨테이너에 연결하는 설정입니다. 예를 들어, `./web:/app/web`는 호스트의 `web` 디렉토리를 컨테이너의 `web` 디렉토리에 연결합니다. 이는 "파일 공유 경로"라고 생각해보세요.
...
1. Docker 설치: [Docker 공식 설치 가이드](https://docs.docker.com/get-docker/) 참고  
2. 설치 확인: `docker --version` 명령어로 설치 여부 확인  
3. 기본 명령어:  
   - `docker run`: 컨테이너 실행  
...
- `chmod -R 777`은 임시로 권한을 조정할 수 있지만, 실제 환경에서는 보안 설정을 고려해야 합니다.  
- 파일 권한은 최소한으로 설정하고, 필요한 사용자만 접근 권한을 부여하는 것이 안전합니다.  

### 🔑 핵심 개념 요약

| 개념 | 설명 | 비유 |
|------|------|------|
| 워킹 디렉토리 | 프로젝트 파일을 저장하는 폴더 | 작업 공간 |
| 바인드 마운트 | 컴퓨터와 컨테이너가 파일을 공유하는 방식 | 파일 공유 |
| 권한 설정 | 파일/디렉토리에 접근할 수 있는 권한 | 파일 접근 권한 |
| watch 모드 | 파일 변경사항이 실시간으로 반영되는 기능 | 실시간 동기화 |