...
**목적**: Docker 이미지를 사용해 실시간 파일 동기화와 개발 서버 실행을 통한 DevOps 환경을 구축하는 실습

**학습 목표**:
- Dockerfile 작성 및 이미지 빌드
- bind mount를 통한 실시간 파일 동기화 설정
- 개발 서버 실행 및 로그 확인
- Docker Compose로 서비스 관리 방법 이해
...
- [이미지 보기](https://docs.example.com/watch-mode-explained.png)
...
Docker Desktop 설치 및 실행
  - 공식 가이드: https://docs.docker.com/get-docker/

AWS CLI 인증 설정
  - 설정 튜토리얼: https://docs.aws.amazon.com/cli/latest/userguide/cli-configure-profiles.html

---

## 👉 Step 1: 프로젝트 디렉토리 생성

**목표**: 작업 디렉토리 생성 및 초기 파일 생성

**명령어**:

```bash
mkdir myapp && cd myapp && touch Dockerfile package.json
...
**문제 해결**:
- 문제: 디렉토리 생성 실패
  해결: sudo 권한으로 재시도 또는 권한 설정 (chmod 777 myapp)
