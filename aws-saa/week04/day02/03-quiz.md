...
A. On-Demand만 유지(할인은 금지)  
B. S3 Glacier Deep Archive  
C. Savings Plans 또는 Reserved Instances  
D. Spot  
<details>
...
- 소거법
  - D (근접 오답): Spot은 “중단 허용” 신호가 있어야 정답이 된다.
  - A (근접 오답): 유연하지만 절감 폭이 작아 “큰 절감” 요구와 어긋난다.
  - B (명확히 틀림): 스토리지 클래스다.
...
- 소거법
  ...
  - A (명확히 틀림): 중단 허용 신호가 있는데 할인을 포기하면 비용 최적화가 약하다.
  - C (명확히 틀림): DNS는 구매 옵션과 무관하다.
...
B. 모든 인스턴스를 가장 큰 타입으로 고정한다  
C. 무조건 Reserved Instances를 산다  
D. 무조건 Spot만 사용한다(중단 고려 없음)  
<details>
<summary>정답/해설</summary>
...
- 소거법
  - C (근접 오답): steady state 신호가 없으면 약정이 함정이 될 수 있다.
  - D (근접 오답): 중단 허용 신호 없이 Spot을 밀면 신뢰성 요구를 깨기 쉽다.
  - B (명확히 틀림): 24시간 비용 낭비가 커진다.
...
A. 보안 그룹을 0.0.0.0/0으로 열면 성능이 좋아진다  
B. CPU만 보고 무조건 한 단계 내린다  
C. 모든 리소스를 밤에 강제로 종료한다(영향 분석 없음)  
D. CloudWatch 지표(CPU/네트워크/I/O 등)로 실제 사용량을 측정한 뒤, 요구사항을 만족하는 최소 스펙을 찾는다  
<details>
<summary>정답/해설</summary>
...
A. CloudWatch 지표로 실제 사용량을 측정해 right sizing(과한 스펙)을 조정한다  
B. Spot을 중단 불가 서비스의 기본 옵션으로 강제한다  
C. 루트 사용자로만 운영 작업을 수행한다  
D. S3 버킷을 퍼블릭으로 연다  
E. Auto Scaling scheduled action으로 야간(비피크)에 desired를 0으로 줄인다  
<details>
<summary>정답/해설</summary>
...
- 소거법
  - B (근접 오답): Spot은 중단 허용 신호가 있어야 한다.
  - D (명확히 틀림): 비용 최적화/보안 모두 악화될 수 있다.
  - C (명확히 틀림): 운영/보안 원칙 위반이다.
