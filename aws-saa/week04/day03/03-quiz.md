...
- 소거법
  - B (근접 오답): 장기 보관/느린 복구(대기 가능) 신호가 강할 때 후보가 된다.
  - C (근접 오답): 단일 AZ 허용 신호가 없으면 위험/요구 미스가 될 수 있다.
  - D (명확히 틀림): 접근 제어를 무너뜨리는 선택이다.
...
A. S3 Standard  
B. S3 Glacier 계열(특히 Deep Archive 같은 장기/최저 비용 방향)  
C. S3 Intelligent-Tiering(항상 정답)  
D. EBS io2  
...
- 소거법
  - A (근접 오답): 즉시/자주 접근 신호가 없으면 과할 수 있다.
  - C (근접 오답): 패턴 예측 어려움 신호가 강할 때 후보가 된다. “항상 콜드”면 명시 선택이 더 자연스럽다.
  - D (명확히 틀림): 블록 스토리지다.
...
C. 모든 데이터를 즉시 Glacier Deep Archive로 이동한다(요구 무시)  
...
- 소거법
  ...
  - C (근접 오답): 비용은 줄 수 있어도 복구 요구/핫 데이터 요구를 깨기 쉽다.
  - D (명확히 틀림): DNS다.
...
B. S3 Glacier Deep Archive(항상 최저)  
C. S3 Standard만 유지(항상 안전)  
D. EFS  
...
- 소거법
  - B (근접 오답): 항상 콜드/복구 느림 OK일 때는 맞지만, ‘갑자기 핫해질 수 있음’ 신호와 충돌할 수 있다.
  - C (근접 오답): 안전하지만 과지출 가능성이 크다(요구가 비용 최적화/자동화 쪽).
  - D (명확히 틀림): 공유 파일시스템이다.
...
A. `logs/` prefix에만 적용되는 Lifecycle rule을 만들어 전환/만료를 자동화한다  
B. prefix/범위를 분리해 데이터 성격별로 다른 정책을 적용한다  
C. 버킷 전체에 동일한 전환을 걸어 모든 데이터를 한 번에 Glacier로 내린다  
D. 수동으로 매달 콘솔에서 옮긴다  
E. S3 버킷을 퍼블릭으로 연다  
...
- 소거법
  - C (근접 오답): 비용은 줄어도 핫 데이터 요구를 깨기 쉬운 함정이다.
  - D (명확히 틀림): 사람 의존/누락으로 사고가 반복된다.
  - E (명확히 틀림): 접근 제어를 무너뜨린다.
...
- S3 클래스는 “가장 싼 것”이 아니라 **액세스 + 복구 시간/비용**으로 고른다.  
- “자동 전환/만료”는 **Lifecycle**, “예측 어려움/자동 최적화”는 **Intelligent-Tiering**.
