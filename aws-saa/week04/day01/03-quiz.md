# 03-quiz - Week 04 Day 01 (Cost Explorer / Budgets / Cost Allocation Tags)
...
B. AWS Budgets (임계치 알림)  
C. Route 53 (DNS)  
D. CloudTrail (누가 무엇을 했나)  
...
- 소거법
  - B (근접 오답): 초과 감지엔 좋지만 “어디서 늘었는지 분석”의 1순위는 아니다.
  - C (명확히 틀림): DNS는 비용 분석 도구가 아니다.
  - D (근접 오답): 변경 원인 추적엔 도움되지만, 비용을 축으로 분해하는 도구는 아니다.
...
- 소거법
  - B (근접 오답): 분석엔 좋지만 임계치 알림 도구는 아니다.
  - C (명확히 틀림): 준수/상태 평가 도구다.
  - D (명확히 틀림): 캐시/전송 계층이다.
...
A. 비용 할당 태그를 표준화하고 활성화해 Cost Explorer에서 태그로 Group by 가능하게 만든다  
B. 모든 리소스를 퍼블릭으로 열어 운영을 단순화한다  
C. CloudFront invalidation을 남발한다  
D. DynamoDB Scan을 늘린다  
...
- 소거법
  - B (명확히 틀림): 보안/비용 모두 악화될 수 있다.
  - C (명확히 틀림): 캐시 운영 포인트지 차지백 해법이 아니다.
  - D (명확히 틀림): DB 조회 방식과 비용 차지백은 무관하다.
...
A. Cost Explorer로 서비스/리전/태그 기준으로 비용 드라이버를 분해한 뒤 우선순위를 잡는다  
B. 가장 큰 리소스를 무조건 종료한다  
C. NAT Gateway를 무조건 삭제한다(서비스 영향은 고려하지 않는다)  
D. 모든 리소스의 권한을 AdministratorAccess로 바꾼다  
...
- 소거법
  - B (명확히 틀림): 비용은 줄 수 있어도 장애로 돌아오기 쉽다.
  - C (근접 오답): NAT가 드라이버일 수는 있지만 “무조건/영향 무시”는 위험하다.
  - D (명확히 틀림): 비용 문제 해결과 무관하고 보안까지 악화된다.
...
- 소거법
  - C (명확히 틀림): 캐시/전송 계층이다.
  - D (명확히 틀림): DNS다.
  - E (근접 오답): 보안 탐지엔 유용하지만 비용 센서/분석 도구는 아니다.
...
- “원인 분석/그룹핑”은 **Cost Explorer**, “임계치/초과 알림”은 **Budgets**, “팀별 비용”은 **태그 표준화**가 먼저다.
