...
A. S3 Gateway VPC Endpoint를 만들고 라우팅 테이블에 연결해 NAT 경유를 피한다  
B. NAT Gateway를 더 큰 사이즈로 바꾸면 비용이 줄어든다  
C. S3 버킷을 퍼블릭으로 열어 NAT 없이 접근한다  
D. Route 53 Failover를 설정해 비용을 줄인다  
...
- 소거법
  - B (명확히 틀림): NAT 비용은 “사이즈 업”으로 자동 절감되지 않는다.
  - C (명확히 틀림): 보안 요구와 정반대다.
  - D (명확히 틀림): DNS는 NAT 비용 드라이버를 제거하지 못한다.
...
다음 중 “프라이빗 서브넷에서 AWS 서비스 접근”을 비용/보안 관점으로 가장 올바르게 설명한 것은?

A. 프라이빗이면 무조건 NAT Gateway가 정답이다(엔드포인트는 필요 없다)  
B. NAT는 “일단 되게” 하지만 비용 드라이버가 될 수 있고, 대상 서비스가 지원하면 VPC Endpoints로 사설 경로를 만들 수 있다  
C. VPC Endpoints는 인터넷을 경유하는 방식이다  
D. VPC Endpoints는 CloudFront의 다른 이름이다  
...
- 소거법
  ...
  - C (명확히 틀림): endpoints는 사설 경로를 만드는 개념이다.
  - D (명확히 틀림): 목적/계층이 다르다.
...
B. Global Accelerator를 붙여 정적 파일을 캐시한다  
...
B. 캐시 키는 무조건 크게 잡아야 hit가 올라간다  
C. invalidation을 모든 배포마다 전 객체에 대해 남발한다  
D. NAT Gateway를 추가로 만든다  
...
- 소거법
  - B (명확히 틀림): 키가 커질수록 변종이 늘어 hit율이 떨어질 수 있다.
  - C (근접 오답): 즉시 반영은 되지만 비용/운영 부담이 커지고 근본 원인(키 폭발)을 해결하지 못한다.
  - D (명확히 틀림): NAT는 CloudFront 캐시 hit율과 무관하다.
...
- 소거법
  - C (근접 오답): 라우팅은 가능하지만 NAT/전송 비용 드라이버를 직접 제거하진 못한다.
  - D (명확히 틀림): 보안 상한선이다.
  - E (근접 오답): 감사엔 유용하지만 비용 드라이버 제거 도구는 아니다.
...
- “프라이빗 + S3 자주 호출 + NAT 비용”이면 **S3 Gateway Endpoint**.  
- “글로벌 + 정적/다운로드 + 전송/요청 비용”이면 **CloudFront**.
