...
- 소거법
  - B (근접 오답): 임계치 알림(센서)에는 좋지만 원인 분석 도구는 아니다.
  - C (명확히 틀림): DNS다.
  - D (명확히 틀림): 보안 탐지다.
...
A. 비용 할당 태그를 표준화/활성화하고 Cost Explorer에서 태그로 Group by 가능하게 만든다  
B. 모두 Spot으로 바꾼다  
C. 모든 데이터를 Glacier Deep Archive로 옮긴다  
D. NAT Gateway를 무조건 삭제한다  
...
- 소거법
  ...
  - C (근접 오답): 복구/접근 요구를 깨기 쉽다.
  - D (근접 오답): NAT가 드라이버일 수는 있지만 “무조건”은 위험하다.
...
- 소거법
  - B (근접 오답): steady state 신호가 더 강할 때 후보.
  - C (근접 오답): 유연하지만 절감 폭이 약하다.
  - D (명확히 틀림): CDN이다.
...
- 소거법
  - C (명확히 틀림): 보안/비용 모두 악화될 수 있다.
  - D (명확히 틀림): 운영/보안 원칙 위반이다.
  - E (명확히 틀림): 비용 낭비다.
...
- 소거법
  - B (근접 오답): 다운로드/정적 콘텐츠 비용 절감 레버이지 NAT 경유 비용 제거는 아니다.
  - C (명확히 틀림): DNS다.
  - D (명확히 틀림): 보안 탐지다.
...
- 분석=Cost Explorer, 알림=Budgets, 팀별=태그.  
- 컴퓨트는 “예측 가능=RI/SP, 중단 허용=Spot, 스파이크=On-Demand+ASG” + “비피크 축소”가 큰 레버.  
- 스토리지는 “빈도+복구 요구” + Lifecycle/Intelligent-Tiering.  
- 네트워크는 “NAT 비용=Endpoints, 글로벌 다운로드 비용=CloudFront”.
