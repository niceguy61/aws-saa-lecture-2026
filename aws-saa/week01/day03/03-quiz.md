# 03-quiz - Week 01 Day 03 (CloudTrail / Config / Detection Services)
...
- 소거법
  - B (근접 오답): Config는 “상태/구성” 이력에 강하지만, “누가 호출했나” 질문에는 CloudTrail이 더 직결된다.
  - C (명확히 틀림): GuardDuty는 탐지(findings) 계층이지, 변경 행위의 원장 로그가 아니다.
  - D (명확히 틀림): Inspector는 취약점/구성 평가 축이다.
...
A. CloudTrail Event history만 사용한다  
B. AWS Config를 사용해 구성 이력과 규칙 기반 준수 평가를 수행한다  
C. WAF를 사용해 HTTP 공격을 차단한다  
...
- 소거법
  - A (근접 오답): 행위 추적에는 유용하지만, 준수/상태 평가 요구를 바로 만족시키기 어렵다.
  - C (명확히 틀림): 웹 공격 방어(L7)로 준수 보고서를 만들 수 없다.
...
- 소거법
  - B (근접 오답): Trail은 장기 보관/감사엔 좋지만, “탐지 엔진”은 아니다.
  - C (명확히 틀림): Config는 준수/상태 평가 축이다.
  - D (명확히 틀림): 저장 클래스는 탐지와 무관하다.
...
- 소거법
  - B (명확히 틀림): 엣지 캐싱/전송 최적화 서비스다.
  - C (근접 오답): 출입구(SSO) 통합이지, findings 집계 허브는 아니다.
  - D (명확히 틀림): 파일 스토리지다.
...
- 소거법
  ...
  - E (명확히 틀림): DDoS 완화 서비스다(행위/준수 질문과 축이 다름).
...
- “누가 했나”는 **CloudTrail**, “준수/규칙 위반”은 **Config**, “탐지/알림”은 **GuardDuty**, “집계”는 **Security Hub**.
