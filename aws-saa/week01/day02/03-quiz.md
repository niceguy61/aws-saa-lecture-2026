...
A. Parameter Store(일반 String)로만 저장하고 수동으로 교체한다  
B. Secrets Manager를 사용해 시크릿을 저장하고(필요 시) rotation을 구성한다  
C. 시크릿을 코드에 하드코딩해 배포한다  
D. 시크릿을 S3에 업로드하고, 필요할 때 다운로드한다  
<details>
...
- 소거법
  - D (명확히 틀림): 저장 위치만 바뀌고, 교체/감사/회수 요구를 만족하기 어렵다.
  - A (근접 오답): 저장은 가능하지만, “회전/운영 기능” 요구가 붙으면 부족해지기 쉽다.
  - C (명확히 틀림): 유출 경로를 스스로 만든다.
...
A. Git 저장소에 암호화해서 커밋한다  
B. EC2 인스턴스에 텍스트 파일로 저장한다  
C. Secrets Manager만 사용한다(항상 정답)  
D. Parameter Store의 SecureString + KMS로 암호화해 저장한다  
<details>
<summary>정답/해설</summary>
...
- 소거법
  - C (근접 오답): 기능은 맞지만 “항상”은 아니다. 요구 기능 대비 과할 수 있다.
  - A (명확히 틀림): 저장소 노출/키 관리/감사 관점에서 위험하다.
  - B (명확히 틀림): 파일 배포/회수/감사가 취약하다.
...
A. EBS 볼륨 타입을 gp3로 바꾼다  
B. Route 53 라우팅 정책을 Latency로 바꾼다  
C. CloudFront 캐시 정책을 조정한다  
D. KMS는 key policy가 관문(gate)일 수 있으므로 key policy에서 호출 주체가 허용되는지 확인한다  
<details>
<summary>정답/해설</summary>
...
C. Route 53에 Weighted 라우팅을 추가한다  
D. 호출 주체(역할/서비스)가 `kms:Decrypt` 및 key policy 관문을 통과하는지 확인한다  
E. CloudFront로 앞단 캐시를 붙여 우회한다  
<details>
<summary>정답/해설</summary>
...
- 소거법
  ...
  - E (근접 오답): 캐시는 증상을 가릴 수 있어도 권한/정책 원인을 해결하지 못한다.
  - C (명확히 틀림): DNS는 권한 거부와 무관하다.
...
A. 비용만 줄이면 보안은 자동으로 좋아진다  
B. 암호화만 켠다(SSE-KMS만 켜면 끝)  
C. 시크릿을 이메일로 공유해 운영을 단순화한다  
D. 암호화 + 접근 제어(최소 권한) + 감사(누가 접근했는지 추적)까지 묶어 설계한다  
<details>
<summary>정답/해설</summary>
...
- 소거법
  - B (근접 오답): 암호화는 필요하지만, 권한/감사가 빠지면 사고 대응이 약하다.
  - A (명확히 틀림): 비용과 보안은 별개 축이다.
  - C (명확히 틀림): 유출 경로를 스스로 만든다.
