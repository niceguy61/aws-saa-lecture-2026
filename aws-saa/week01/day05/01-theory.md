...
![고객 사례 삽화 - Week 1 보안 요약](../../assets/scenario_image/w1d5s1.png)

한 기업이 M&A를 하면서 AWS 계정이 12개가 됐다. 각 팀은 속도가 중요해서 자기 방식대로 계정을 운영했고, 어느 순간부터 “어느 계정에 어떤 권한이 있는지” 아무도 확신을 못 한다. 사고가 나면 CloudTrail은 계정마다 따로 있고, 시크릿은 Parameter Store/Secrets Manager/S3/서버 파일에 섞여 있다. 네트워크는 NAT로 우회한 계정도 있고, 엔드포인트로 닫아둔 계정도 있다. 보안팀은 “표준을 만들고, 최소 권한으로 통제하고, 감사 가능하게”를 요구하지만, 현장은 “개발이 안 멈추게”가 더 중요하다.

이때 Week 1의 핵심은 ‘서비스 목록’이 아니라 ‘조합 규칙’이다. 접근은 Role+STS로 키 공유를 없애고, 상한선은 SCP/permission boundary로 먼저 걸어둔다. 시크릿/암호화는 KMS를 중심으로 관리형 저장소로 모으고, 사건이 나면 CloudTrail/Config로 행위와 상태를 분리해서 추적한다. 프라이빗 네트워크는 Endpoints/PrivateLink로 인터넷 경유를 줄인다. 이렇게 하면 “각 팀이 빠르게 움직여도, 위험한 선택지는 구조적으로 못 하게” 만들 수 있다.

당신 조직이 지금 가장 먼저 고정해야 할 축은 권한/암호화/감사/네트워크 중 무엇인가요?
