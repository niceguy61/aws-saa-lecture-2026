# 03-quiz - Week 01 Day 05 (Week Summary / Domain 1)
...
A. 팀 공용 IAM 사용자 키를 공유한다  
B. Role을 만들고 STS `AssumeRole`로 임시 자격 증명을 사용한다  
C. 루트 사용자만 쓰도록 강제한다  
D. S3를 퍼블릭으로 열어 인증 없이 접근한다  
...
- 소거법
  - A (명확히 틀림): 키 공유는 회수/감사/노출에 취약하다.
  - C (근접 오답): 강해 보이지만 통제/감사/권한 분리 관점에서 오답이다.
...
B. 파트너에게 IAM 사용자 액세스 키를 발급해 전달한다  
C. Role 권한을 최소 권한으로 제한하고, 필요 시 session policy로 더 좁힌다  
D. S3 버킷을 퍼블릭으로 열어 인증을 제거한다  
E. 루트 사용자만 사용하도록 강제한다  
...
- 소거법
  ...
  - D (명확히 틀림): 인증 제거는 요구사항과 정반대다.
  - E (명확히 틀림): 루트는 통제/감사/분리 관점에서 오답이다.
...
A. SCP는 권한을 부여하므로, SCP Allow만 있으면 IAM 없이도 된다  
B. SCP는 OU/계정 단위 상한선이며, SCP에서 막히면 IAM Allow로 뚫을 수 없다  
C. SCP는 특정 S3 버킷에만 적용된다  
...
- 소거법
  - A (근접 오답): 시험 단골 함정. SCP는 권한 부여가 아니다.
  - C (명확히 틀림): 조직 단위다.
...
B. CloudFront 캐시를 지운다  
C. Route 53 라우팅을 Weighted로 바꾼다  
D. S3 버킷을 퍼블릭으로 연다  
...
- 소거법
  - B (명확히 틀림): 캐시는 권한을 해결하지 못한다.
  - C (명확히 틀림): DNS/라우팅은 KMS 권한과 무관하다.
  - D (명확히 틀림): 인증 제거는 요구사항과 정반대다.
...
- 소거법
  ...
  - C (명확히 틀림): 감사/행위 로그다.
  - D (근접 오답): 단순 저장엔 좋지만 rotation 요구가 있으면 부족해지기 쉽다.
...
C. 호출 주체가 `kms:Decrypt` 및 key policy 관문을 통과하는지 확인한다  
D. Route 53 레코드를 바꾼다  
E. S3 버킷을 퍼블릭으로 연다  
...
- 소거법
  - C (명확히 틀림): 캐싱/전송 계층이다.
  - D (근접 오답): DNS/라우팅은 준수 평가와 무관하다.
  - E (명확히 틀림): 웹 공격 방어(L7)다.
...
- 소거법
  - B (근접 오답): Trail은 장기 보관/감사엔 좋지만 탐지 엔진이 아니다.
  - C (명확히 틀림): 저장 클래스다.
  - D (명확히 틀림): 준수/상태 평가 축이다.
...
- 소거법
  - C (근접 오답): 인터넷을 쓰는 방향으로 요구사항과 반대다.
  - D (명확히 틀림): DNS는 사설 경로를 만들지 않는다.
  - E (명확히 틀림): 인증 제거는 요구사항과 정반대다.
...
- “키 공유/루트”는 소거하고, **Role + STS AssumeRole**을 기본으로 깐다.  
- “SSE-KMS AccessDenied”는 **KMS(키/키 정책/Decrypt)**까지 같이 본다.  
- “누가 했나=CloudTrail, 준수/상태=Config, 탐지/알림=GuardDuty”로 계층을 분리한다.
