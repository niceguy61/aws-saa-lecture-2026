...
C. EBS 성능 축(IOPS/처리량) 병목을 의심하고 볼륨 타입/튜닝(gp3/io2)을 검토한다  
D. EC2 인스턴스 타입만 계속 키운다(컴퓨트가 항상 병목이다)  
<details>
...
- 소거법
  - D (근접 오답): 흔한 오판. 컴퓨트 스케일업만으로 I/O 병목이 풀리지 않을 수 있다.
  - B (명확히 틀림): DNS는 EBS 큐 길이와 무관하다.
  - A (명확히 틀림): 권한 문제는 I/O 지표로 나타나지 않는다.
...
A. standard(magnetic)로 바꿔 레거시를 유지한다  
B. st1(sc1) 같은 HDD로 바꿔 비용을 낮춘다  
C. gp3를 사용하고 IOPS/처리량을 조절해 튜닝한다  
...
- 소거법
  - A (명확히 틀림): 성능 요구와 반대다.
  - B (근접 오답): 가능할 수는 있지만 “일관된 고IOPS” 신호엔 io2가 더 자연스럽다.
  - D (명확히 틀림): 객체 스토리지다.
...
A. 각 인스턴스에 파일을 따로 유지하고 “문제 없다고 가정”한다  
B. Route 53 Latency 라우팅으로 파일을 공유한다  
C. EBS를 여러 인스턴스에 동시에 붙인다  
D. EFS를 마운트해 공유 파일시스템으로 사용한다  
<details>
<summary>정답/해설</summary>
...
- 소거법
  - C (근접 오답): EBS는 기본적으로 인스턴스에 붙는 블록 스토리지라 “공유 모델”과 결이 다르다(함정).
  - A (명확히 틀림): 요구사항(공유/일관성)과 정면 충돌한다.
  - B (명확히 틀림): DNS는 파일 일관성을 제공하지 않는다.
...
고객 사례 회고에서 두 가지 문제가 동시에 나왔다.  
- 문제 1: “CPU는 낮은데 느리다” + `VolumeQueueLength` 상승(스토리지 I/O 병목 신호)  
- 문제 2: Auto Scaling 이후 업로드 파일이 인스턴스마다 달라져 일관성이 깨짐(공유 파일시스템 신호)  
팀이 EBS/EFS를 혼동하지 않도록, 아래 보기 중 ‘옳은 설명’ 2개를 고르시오.
A. gp3는 공유 파일시스템이라 여러 인스턴스가 동시에 마운트한다  
B. EBS는 인스턴스에 붙는 블록 스토리지로 “단일 인스턴스 I/O 튜닝” 축에 가깝다  
C. EFS는 “용량은 그대로, IOPS만 조절” 같은 요구에 가장 직접적이다  
D. NACL을 열면 EBS IOPS가 자동으로 증가한다  
E. EFS는 여러 인스턴스가 동시에 마운트해 공유하는 파일시스템(NFS) 축에 가깝다  
<details>
<summary>정답/해설</summary>
...
- 소거법
  ...
  - A (명확히 틀림): gp3는 EBS 볼륨 타입이다.
  - D (명확히 틀림): 네트워크 규칙과 IOPS는 별개다.
