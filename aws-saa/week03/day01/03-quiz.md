...
A. T(버스트) 계열을 더 늘린다(항상 가장 싸다)  
B. C(컴퓨트 최적화) 계열 같은 지속 고CPU에 맞는 패밀리를 고려한다  
C. X(초고메모리) 계열로 바꾼다(무조건 빠르다)  
D. S3 Versioning을 켠다  
...
- 소거법
  - A (근접 오답): 스케일로 버틸 수 있어 보여도, 크레딧 소진 유형이면 근본 원인을 못 고친다.
  - C (명확히 틀림): 메모리 신호 없이 초고메모리를 고르는 건 과하다.
  - D (명확히 틀림): 스토리지 실수 복구 기능이다.
...
A. CPU 병목(스펙 업만 하면 해결)  
B. 스토리지 I/O 병목(EBS 큐/처리량 이슈)  
C. DNS 병목(Route 53)  
D. IAM 권한 평가 병목(Explicit Deny)  
...
- 소거법
  - A (근접 오답): CPU만 보고 결론 내리는 건 흔한 함정이다.
  - C (명확히 틀림): DNS는 볼륨 큐 길이와 무관하다.
  - D (명확히 틀림): 권한 문제는 지표(QueueLength)로 나타나지 않는다.
...
A. T 계열은 지속 부하에서 크레딧 소진으로 스로틀링되어 성능이 급락할 수 있다  
B. T 계열은 언제나 일관된 성능을 보장한다  
C. CloudTrail이 부족해서 지연이 생긴다  
D. S3 Replication이 설정되지 않아 지연이 생긴다  
...
- 소거법
  - B (근접 오답): 반대다. 일관 성능 요구에 T는 함정이 될 수 있다.
  - C (명확히 틀림): 감사 로그는 성능과 축이 다르다.
  - D (명확히 틀림): 복제는 스토리지 DR 축이다.
...
- 소거법
  - C (근접 오답): 네트워크 병목일 수도 있지만, “T 크레딧” 확인이라는 목적에 더 직접적이지 않다.
  - D (근접 오답): I/O 병목 진단에 유용하지만, 크레딧 소진 여부를 바로 말해주진 않는다.
  - E (명확히 틀림): 인스턴스 상태 체크는 크레딧 소진과 다른 축이다.
...
- 성능 문제는 “더 큰 인스턴스”가 아니라 **병목 축 분리 + 지표 교차 확인**으로 푼다.  
- T 계열은 “간헐 스파이크”에 좋지만, “지속 부하/일관 성능” 신호에선 함정이 될 수 있다.
