...
![고객 사례 삽화 - EC2 선택과 T 크레딧 함정](../../assets/scenario_image/w3d1s1.png)
...
### EC2 인스턴스 패밀리/타입 전체 정리(시험 기준)

> 시험에서는 “정확한 세부 모델명”보다 **문장 신호 → 패밀리 범주** 매칭을 더 자주 본다. 아래 표에서 표준/대표 선택지는 **굵게** 표시했다.

| 범주 | 대표 패밀리(예시) | 언제 쓰나(문장 신호) | 비용 효율 포인트 |
|---|---|---|---|
| 범용(표준) | **M** | 특별한 신호 없이 “균형형” | 기본값으로 안전(최신 세대 선호) |
| 버스트(저렴하지만 조건부) | T | 간헐 스파이크, dev/test | 지속 부하에는 크레딧 함정(아래) |
| 컴퓨트 최적화 | **C** | 지속 고CPU, CPU가 병목 | 지연/처리량이 CPU에 민감할 때 효율적 |
| 메모리 최적화 | **R** | 메모리/캐시/인메모리 힌트 | 메모리 부족(OOM) 신호가 강하면 후보 |
| 초고메모리/특수 | X/Z 등 | 대규모 인메모리/특수 워크로드 | 비용↑, 신호가 명확할 때만 |
| 스토리지 최적화 | I/D/H 등 | 로컬 디스크 I/O/처리량 힌트 | 성능은 좋지만 비용/운영 제약 확인 |
| 가속(특수) | G/P/F, Inf/Trn 등 | GPU/그래픽/ML/추론/학습 신호 | 비용↑, 문제 문장에 명시 신호가 나옴 |
| 아키텍처(비용/성능) | `*g`(Graviton) | “가격 대비 성능”, “Arm 호환” 힌트 | 호환되면 비용 효율이 좋아지는 경우가 많다 |

### 비용 드라이버/비용 효율 포인트(컴퓨트 관점)

- “T가 싸다”는 **burst 조건에서만** 의미가 있다(지속 부하면 오히려 비용/운영이 악화될 수 있다).
- 같은 범주의 패밀리라도 **세대/아키텍처(예: `*g`)**에 따라 비용 효율이 달라질 수 있다(호환성 확인).
- 스펙 업 1번보다, 스파이크/비피크가 명확하면 **ASG/스케줄 축소(비피크 0)**가 더 큰 절감이 되기도 한다(Week04와 연결).
