...
- V(Visual): Query vs Scan 다이어그램으로 ‘빠른 길/느린 길’을 본다.
- A(Auditory): “Query=키 기반, Scan=전체 탐색, GSI=새 패턴”을 말로 고정한다.
- O(Olfactory, smell test): Scan/핫 파티션/특정 키 집중은 냄새가 난다.
- G(Gustatory, taste test): 문장 1개 보고 Query vs GSI를 고른다.
...
- Exam must-know
  - Key point: Query가 가능한 상황에서 Scan은 거의 오답 후보가 된다.
  - Why: Scan은 전체를 읽기 때문에 데이터가 커질수록 지연/비용이 폭증한다.
  - Alternative: 원하는 조회 조건이 키로 안 나오면 GSI로 액세스 패턴을 추가한다.
