# 04-quiz - Week 03 Day 04 (DynamoDB / ElastiCache / Aurora)
...
C. 새로운 액세스 패턴을 위해 GSI(Global Secondary Index)를 추가한다  
D. CloudFront를 붙여 DynamoDB를 캐시한다(무조건 정답)  
<details>
...
- 소거법
  ...
  - D (근접 오답): 캐시는 반복 읽기엔 유용하지만, 키 설계/조회 경로 문제를 근본 해결하지 못한다.
  - B (명확히 틀림): DNS는 DB 조회 경로를 바꾸지 않는다.
...
B. CloudTrail 로그가 부족  
C. NACL이 stateful이라서 리턴 트래픽이 막힘  
D. S3 Versioning 미설정  
<details>
<summary>정답/해설</summary>
...
- 소거법
  ...
  - C (명확히 틀림): NACL은 stateless다(그리고 DynamoDB 핫 파티션과 무관).
...
A. Route 53 Weighted로 트래픽을 나눈다  
B. DynamoDB Scan을 늘려서 해결한다  
C. 모든 데이터를 항상 최신으로 강제하고 캐시는 금지한다  
D. ElastiCache로 읽기 핫패스를 캐시한다  
<details>
<summary>정답/해설</summary>
...
- 소거법
  ...
  - A (명확히 틀림): DNS는 DB 읽기 핫패스를 줄이지 못한다.
...
A. 모든 읽기 문제는 ElastiCache로만 해결한다(항상 정답)  
B. DynamoDB로 무조건 전환한다(관계형 요구 무시)  
C. Aurora/RDS Read replica(읽기 확장)로 읽기 트래픽을 분산한다  
D. Multi-AZ만 붙이면 읽기 성능이 자동으로 늘어난다  
<details>
<summary>정답/해설</summary>
...
- 소거법
  - A (근접 오답): 캐시는 효과가 크지만, 최신성/무효화/데이터 특성에 따라 정답이 갈린다(“항상”은 함정).
  - D (근접 오답): Multi-AZ는 HA(failover) 목적이지 read scaling 도구가 아니다.
  - B (명확히 틀림): 요구사항(관계형 기능)을 무시하는 건 오답이다.
...
고객 사례 회고에서 팀이 세 가지 축을 한꺼번에 섞어 결론을 내렸다.  
- DynamoDB 조회는 Query/Scan/GSI를 구분하지 못해 비용/지연이 튐  
- 반복 읽기 API는 캐시를 고려하지 않아 DB 부하가 계속 올라감  
- Aurora 읽기 지연을 “Multi-AZ만 붙이면 해결”이라고 오해함  
아래 보기 중, 팀에 “핵심 규칙”으로 전달하기 좋은 옳은 설명 2개를 고르시오.
A. DynamoDB에서 Query는 키 기반 빠른 경로이고, Scan은 전체 탐색이라 데이터가 커질수록 느리고 비싸진다  
B. GSI는 “캐시 무효화 정책”을 의미한다  
C. ElastiCache는 반복 읽기 핫패스에서 DB 호출 수를 줄여 지연을 낮출 수 있다(단, 최신성/무효화 트레이드오프)  
...
- 소거법
  - B (명확히 틀림): GSI는 새로운 액세스 패턴(인덱스)이다.
