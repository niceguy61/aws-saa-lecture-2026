...
- 소거법
  ...
  - C (명확히 틀림): 캐시/전송 계층이다.
  - D (명확히 틀림): DNS는 객체 버전 복구를 제공하지 않는다.
...
- 소거법
  - A (근접 오답): 복제는 맞는 기술이지만 요구(실수 복구)에 비해 과하고 비용이 늘 수 있다.
...
A. 소스/대상 버킷 모두 Versioning을 켠다  
B. S3 Cross-Region Replication(CRR)을 설정한다  
C. CloudFront를 앞단에 둔다  
...
- 소거법
  - C (근접 오답): 성능/전송 최적화에는 도움되지만 “원격 복제” 요구를 만족시키지 않는다.
  - D (명확히 틀림): DNS 전환은 데이터 복제를 만들지 않는다.
  - E (명확히 틀림): 접근 제어/보안 요구와 반대다.
...
- “실수 복구”면 **S3 Versioning**, “리전 DR/규제”면 **S3 Replication(CRR/SRR)**(+ Versioning 전제).  
- “디스크 데이터 복구”면 **EBS Snapshot**.
