...
C. Route 53 Weighted 라우팅을 설정한다  
D. Read replica를 추가해 읽기 트래픽을 분산한다  
<details>
<summary>정답/해설</summary>
...
- 소거법
  ...
  - B (명확히 틀림): 네트워크를 여는 건 DB 읽기 확장을 만들지 못한다.
  - C (명확히 틀림): DNS는 DB 내부 읽기 확장과 축이 다르다.
...
팀이 “읽기 지연이면 Multi-AZ를 붙이면 되죠?”라고 말하며 변경을 진행하려 한다. 운영팀은 ‘가용성(자동 failover)’과 ‘읽기 확장(read scaling)’을 헷갈리면 장애/비용이 커진다고 경고한다.  
이 혼동을 가장 정확히 바로잡는 문장은?
A. 두 기능은 모두 DNS 라우팅 정책이다  
B. Multi-AZ는 읽기 성능을 늘리는 기능이므로 read-heavy 문제에 1순위다  
C. Read replica는 자동 장애 조치(HA)용 standby이므로 Multi-AZ를 대체한다  
D. Read replica는 ‘읽기 확장’, Multi-AZ는 ‘자동 장애 조치(가용성)’로 목적이 다르다  
<details>
<summary>정답/해설</summary>
...
- 소거법
  - C (근접 오답): “read replica로 HA”는 단골 오답이다.
  - B (근접 오답): 목적이 반대다.
  - A (명확히 틀림): DB 옵션이지 DNS가 아니다.
...
C. Route 53 Failover  
D. DynamoDB PITR(Point-in-time recovery)  
<details>
<summary>정답/해설</summary>
...
- 소거법
  - A (명확히 틀림): 캐시는 데이터 복구를 제공하지 않는다.
  - C (명확히 틀림): DNS 전환은 데이터 롤백이 아니다.
  - B (근접 오답): 복제는 원격 DR/규제에서 등장한다. “시점 복구” 요구와 축이 다르다.
...
- 소거법
  - A (명확히 틀림): 객체 스토리지 실수 복구다.
  - C (명확히 틀림): 로드밸런서는 DB의 failover/read scaling을 직접 제공하지 않는다.
  - D (근접 오답): 감사/추적엔 필요할 수 있지만, 가용성/읽기 확장 요구의 답이 아니다.
